...
public class Logger: NSObject {
    ...
    public var output: LoggerOutput = .debuggerConsole
    ...
    private func log(_ level: LoggerLevel, text: String, currentTime: Date, persistNow: Bool = false) {
        guard level.rawValue >= self.level.rawValue else { return }
        switch self.output {
            case .fileOnly:
                addToBuffer(text: "\(currentTime.currentDateString) \(text)", persistNow: persistNow)
            case .debuggerConsole:
                print("\(currentTime.currentDateString) \(text)")
            case .deviceConsole:
                NSLog(text)
            case .debugerConsoleAndFile:
                print("\(currentTime.currentDateString) \(text)")
    ...
    private func log(_ level: LoggerLevel, text: String, persistNow: Bool = false) {
        guard level.rawValue >= self.level.rawValue else { return }
        switch self.output {
            case .fileOnly:
                addToBuffer(text: text, persistNow: persistNow)
            case .debuggerConsole:
                print(text)
            case .deviceConsole:
                NSLog(text)
...
extension Logger {
    ...
    public func dw(_ message: String, currentTime: Date = Date()) {
        log(.debug, text: message, currentTime: currentTime)
    }
    
    // 纯日志输出
    public func dwd(_ message: String) {
